...
Predictive coding frameworks such as Active Inference or Kolmogorov Theory
%, which propose that neural computations rely on hierarchical error minimization, where incoming sensory signals are compared against internal model predictions, 
require a neural hierarchical information encoding scheme. 
% Here, we introduce Hierarchical Amplitude Modulation (HAM) information encoding as a mechanism for predictive processing, and implement it within the Laminar Neural Mass Model (LaNMM) that integrates infragranular Jansen–Rit (JR) and supragranular Pyramidal-Interneuronal Network Gamma (PING) subpopulations. We show how  LaNMM can be used to implement modulation and demodulation schemes that allow oscillatory signals to be dynamically encoded as amplitude envelopes and subsequently reconstructed through filtering and nonlinear interactions. We then show how HAM describes a nested encoding scheme in which oscillatory signals and their amplitude envelopes iteratively modulate each other across multiple timescales, enabling a dynamic interplay between signal representation and demodulation.
%  Notably, this enables a principled account of the logarithmic spacing of neural oscillations and oscillatory power laws.
% \end{abstract}
%\begin{abstract}
We propose \emph{Hierarchical Amplitude Modulation} (HAM) as a neural encoding
scheme in which faster oscillations are iteratively amplitude‑modulated by slower
processes, forming a demodulable cascade. The multiplicative structure generates
intermodulation components at \(f_c+\sum a_i f_i\) and favors \emph{geometric}
(log‑spaced) band centers to avoid spectral overlap, i.e., a constant‑\(Q\) arrangement
akin to wavelet filterbanks. A simple analysis shows how such cascades can produce
\(1/f^\alpha\)‑like aperiodic spectra with
\(\alpha=2\ln(2/m)/\ln r\) under log spacing (ratio \(r>1\)), a candidate mechanism for aperiodic structure. As a proof of concept, we implement amplitude modulation and demodulation in a laminar
...
\begin{itemize}
\item \textbf{HAM:} A hierarchical encoding scheme where faster rhythms are multiplicatively modulated by slower processes; information can live in signals or their envelopes (and envelopes‑of‑envelopes).
\item \textbf{Guard‑bands \& constant‑Q:} The intermodulation structure $f_c \pm \sum a_i f_i$ favors geometric (log) spacing, yielding constant fractional bandwidths and staged demodulation.
\item \textbf{Two routes to $1/f^\alpha$:} (i) a cascade link $\alpha = 2\ln(2/m)/\ln r$; (ii) a log‑uniform mixture that yields $1/f$ in expectation with constant‑Q kernels.
\item \textbf{Mechanism in LaNMM:} PING‑like fast generators as carriers; JR‑like slow generators for envelope extraction; practical demodulation via analytic‑signal envelopes.
\end{itemize}
...
a geometric progression with a common ratio \(r \approx 3\), i.e.,
\(f_{k+1}=f_k/r\) with \(r>1\) (equivalently, equal spacing on a
...
\begin{figure} [t]
    ...
    \caption{Logarithmic hierarchy, adapted from Penttonen \& Buzsáki (2003).
Oscillation bands are approximately log‑spaced with common ratio \(r>1\)
so that \(f_{k+1}=f_k/r\) (empirically \(r\approx 3\) between neighboring
bands). This is equivalent to equal spacing on a log‑frequency axis and
corresponds to a constant‑Q arrangement \cite {penttonenNaturalLogarithmicRelationship2003}.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\paragraph{Graph-based HAM (sum–then–modulate; unified form).}
...
A simple motif that meets \eqref{eq:superincreasing} for deep cascades is a geometric progression
\begin{equation}
...
\(\sum_{j>k} f_j=\frac{f_k}{r-1}\!\bigl(1-r^{-(N-k)}\bigr)\), hence \(r\simeq 2\) is typically sufficient in practice for non-overlap, with slack set by modulation bandwidths and indices.
...
\begin{figure}[t!]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/ham_signals.png}
    \includegraphics[width=0.9\linewidth]{figures/ham_spectrum.png}
	...
	1.	Envelope Modulations (First three Subplots): Each line represents an envelope function of the form  $1 + m_i \cos(2\pi f_i t + \phi_i)$, where each modulation frequency  $f_i$  is progressively lower. These envelopes modulate all previously applied layers, creating an intricate structure of amplitude variations at multiple timescales.
	2.	Final Hierarchical AM Signal (demeaned): The resulting signal after applying all modulation layers. This waveform is the product of all envelope layers modulating a carrier, generating a complex multi-scale amplitude variation.
...
\(\Rightarrow\) shallower slope, see Figure~\ref{power_law}). %This is a multiplicative‑cascade result for HAM under log spacing, not a universal account of aperiodic structure.


\begin{figure} [t!]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/power_law.png}
\caption{\textbf{Power‑law exponent \(\alpha\) for log‑spaced HAM.}
...
that satisfies the weaker \emph{super-increasing} condition
\[
f_k>\sum_{j>k}f_j
\]
implies that the ratios \(r_k=f_k/f_{k+1}\) are all \(>2\), though not necessarily
constant. This is because
$$
r_k=\frac{f_k}{f_{k+1}}> \frac{\sum_{j>k}f_j}{f_{k+1}}= 1 + \frac{\sum_{j>k+1}f_j}{f_{k+1}} >2
$$
...
We introduced \emph{Hierarchical Amplitude Modulation} (HAM) as a compact encoding scheme in which faster rhythms are multiplicatively modulated by slower processes, forming a demodulable cascade. The key technical points are:
\begin{enumerate}
    \item A simple spacing requirement for \emph{non‑overlap} of intermodulation clusters—formalized via a super‑increasing constraint—naturally admits \emph{geometric (log) spacing} as a convenient solution with constant‑\(Q\) benefits. This mirrors classical filterbanks and cochlear tiling (\href{https://www.ee.columbia.edu/~dpwe/papers/Brown91-cqt.pdf}{Brown, 1991}).
    \item Two distinct routes to aperiodic scaling: (i) a \emph{cascade} mechanism that links \(\alpha\) to spacing \(r\) and modulation depth \(m\), \(\alpha = 2\ln(2/m)/\ln r\) (toy construction), and (ii) a \emph{mixture} mechanism in which a log‑uniform ensemble of oscillators (equal density per octave) already yields \(S(f)\propto 1/f\) in expectation. The latter accords with parameterizations that separate periodic and aperiodic structure (\href{https://www.nature.com/articles/s41593-020-00744-x}{Donoghue et al., 2020}; review \href{https://pmc.ncbi.nlm.nih.gov/articles/PMC4149861/}{He, 2014}).
    \item Random AM pairings over a log‑uniform pool add sidebands whose \emph{expected} densities preserve the \(1/f\) backbone in the interior of the band, with two interpretable edge effects: a low‑\(f\) \emph{pedestal} from small differences and a high‑\(f\) \emph{roll‑off} from finite support; both modulate fitted \(\hat\alpha\) depending on analysis choices (band edges, bandwidths).
\end{enumerate}

 
HAM provides a clean account of how slow processes (alpha/beta/theta) can \emph{gain‑modulate} faster carriers (beta/gamma) and be \emph{demodulated} downstream. This dovetails with the spectral segregation of feedforward and feedback channels—gamma for FF content; alpha/beta for FB predictions—in primate laminar circuits (\href{https://www.pnas.org/doi/10.1073/pnas.1402773111}{van Kerkoerle et al., 2014}; \href{https://pubmed.ncbi.nlm.nih.gov/25556836/}{Bastos et al., 2015}; \href{https://pmc.ncbi.nlm.nih.gov/articles/PMC4871751/}{Michalareas et al., 2016}). In our LaNMM instantiation, PING‑like fast and JR‑like slow generators implement, respectively, “carrier” and “envelope” roles with straightforward demodulation via analytic‑signal envelopes (\href{https://www.math.ucdavis.edu/~saito/data/sonar/boashash1.pdf}{Boashash, 1992}).


 \paragraph{An explanation for approximately logarithmic spacing.}
We started from engineering constraints: sidebands from products \(\sum a_i f_i\) remain separable if each cluster around \(\pm f_k\) exceeds the span of slower combinations (\(f_k > \sum_{j>k} f_j\)). A geometric progression \(f_{k+1}=f_k/r\) satisfies this with room to spare for \(r\gtrsim 2\) (and yields uniqueness for signed combinations for \(r>3\)). The same progression gives constant fractional bandwidths (constant‑\(Q\)), enabling a \emph{scale‑invariant} demodulation chain. The coding‑theoretic analogy is to Kraft–McMillan: the inequality \(\sum_{j>k} f_j < f_k\) plays the role of a prefix‑free feasibility bound for “code‑lengths” on the log‑frequency axis (\href{https://projecteuclid.org/journals/annals-of-mathematical-statistics/volume-27/issue-2/The-basic-theorems-of-information-theory/10.1214/aoms/1177728174.full}{McMillan, 1956}; see also \href{https://onlinelibrary.wiley.com/doi/book/10.1002/047174882X}{Cover \& Thomas}).

\paragraph{Two routes to \(1/f^\alpha\): what each explains:}


\paragraph{(A) Multiplicative cascade (HAM).} Under log spacing with common \(r\) and small, approximately constant \(m\), the product structure assigns amplitudes \(\propto (m/2)^k\) to components involving \(k\) modulators. Because \(k\sim \log_r(f_1/f)\), the spectrum exhibits \(P(f)\sim f^{-\alpha}\) with \(\alpha=2\ln(2/m)/\ln r\). This is a \emph{mechanistic} link between slope and circuit variables \((r,m)\) within a chain.

\paragraph{(B) Mixture of log‑spaced oscillators.} If centers follow \(\rho(f)=C/f\) and each contributes equal power with narrow constant‑\(Q\) kernels, the summed baseline is \(S_0(f)=\int \rho K \approx C/f\) (equal energy per octave \(\Rightarrow\) pink). Random AM sidebands from i.i.d.\ draws contribute \(D(f)\approx \kappa_{-}(f)/f\) and \(U(f)\approx \kappa_{+}(f)/f\) in the interior (slowly varying log factors), plus a low‑\(f\) constant pedestal and a high‑\(f\) truncation. Depending on fit range and bandwidths, sidebands can flatten or steepen the apparent \(\hat\alpha\). This clarifies why our simulations show small, analysis‑dependent shifts in slope. Related accounts—mixtures of time scales, synaptic filtering and E/I—are well documented (\href{https://pubmed.ncbi.nlm.nih.gov/28676297/}{Gao et al., 2017}; \href{https://arxiv.org/abs/physics/0204033}{Milotti, 2002}; \href{https://pmc.ncbi.nlm.nih.gov/articles/PMC2711281/}{Bédard \& Destexhe, 2009}).
Finally, although AM generates symmetric lines at \(f_c\pm f_m\), a log‑uniform \(\rho(f)\propto 1/f\) implies that \emph{per absolute Hz} both lower and upper sidebands contribute \(\sim 1/f\) in the band interior. The \emph{asymmetries that matter} are edge effects: (i) small differences accumulate into a finite low‑\(f\) pedestal; (ii) upper sidebands are truncated at \(F_{\max}\). Which effect dominates depends on the analysis window and smoothing (constant‑\(Q\) vs constant‑Hz). This reconciles intuition about “low‑frequency enrichment” with the analytic expectations and our simulations.

\subsection*{Predictions and falsifiability}
\begin{enumerate}
    \item \textbf{Intermodulation arithmetic.} Frequency‑tagging with two or three drives will reveal sum/difference lines \(n_1 f_1 + n_2 f_2 (+n_3 f_3)\) if HAM‑like nonlinear mixing/demodulation is engaged; amplitudes scale with products of modulation depths. (SSVEP/MEG precedents: \href{https://jov.arvojournals.org/article.aspx?articleid=2291652}{Norcia et al., 2015}; \href{https://www.sciencedirect.com/science/article/abs/pii/S1053811919304938}{Gordon et al., 2019}; \href{https://www.sciencedirect.com/science/article/pii/S1053811924004348}{Chen et al., 2024}.)
    \item \textbf{Spacing constraint.} Sideband clusters do not overlap if the bands are roughly geometric with \(r\gtrsim 2\); departures (linear spacing) increase collisions. A constant‑\(Q\) analysis should show cleaner demodulation with geometric than with linear spacing.
    \item \textbf{Laminar dissociation.} Superficial gamma “carriers” and deeper alpha/beta “envelopes” dissociate in directionality (FF vs FB) and in demodulation read‑outs across layers (\href{https://www.pnas.org/doi/10.1073/pnas.1402773111}{van Kerkoerle et al., 2014}; \href{https://pubmed.ncbi.nlm.nih.gov/25556836/}{Bastos et al., 2015}; \href{https://pmc.ncbi.nlm.nih.gov/articles/PMC4871751/}{Michalareas et al., 2016}).
    \item \textbf{Aperiodic slope vs \((r,m)\).} Within a chain, manipulating modulation depth (e.g., via E/I‑targeted pharmacology) should change \(\hat\alpha\) in the direction predicted by \(\alpha(r,m)\). Across populations, changing the band density (effective \(\rho(f)\)) shifts baseline slope toward/away from \(1/f\).
    \item \textbf{Demodulation cascade.} Iterated analytic envelopes (envelope‑of‑envelope) expose log‑spaced content at predicted centers with decreasing \(Q\).
\end{enumerate}

\subsection*{Relation to criticality and renormalization ideas}
Power laws and hierarchical spacings invite comparisons to critical phenomena and renormalization. HAM provides a \emph{constructive} route to fractal‑like spectra without invoking a phase transition; however, signatures of near‑critical dynamics—neuronal avalanches, scale‑invariant correlations—may coexist and interact (\href{https://www.jneurosci.org/content/23/35/11167}{Beggs \& Plenz, 2003}; \href{https://pubmed.ncbi.nlm.nih.gov/22627091/}{Shew \& Plenz, 2013}; \href{https://doi.org/10.1103/PhysRevLett.123.178103}{Meshulam et al., 2019}). Distinguishing these requires perturbational tests and finite‑size scaling, not just spectral fits.

\subsection*{Methodological notes and limitations}
\begin{itemize}
    \item \textbf{Separating periodic from aperiodic.} Use parameterizations (e.g., FOOOF) to avoid conflating oscillatory peaks with the aperiodic slope, and report confidence intervals for \(\hat\alpha\) (\href{https://www.nature.com/articles/s41593-020-00744-x}{Donoghue et al., 2020}).
    \item \textbf{PAC pitfalls.} Estimation depends on bandwidth choices and surrogates; avoid spurious CFC (\href{https://pubmed.ncbi.nlm.nih.gov/24447842/}{Dvořák \& Fenton, 2014}).
    \item \textbf{Bandwidth choice.} Constant absolute smoothing biases high‑\(f\); prefer constant‑\(Q\) when testing scale‑invariance.
    \item \textbf{Model scope.} Our multiplicative toy ignores phase noise, nonstationarity, and spatial mixing; LaNMM covers some of this but remains a reduced model.
\end{itemize}
...
% HAM casts neural activity as a nested, demodulable code in which slower processes scale the gain of faster carriers across a hierarchy of bands. This picture is compatible with the empirical organization of rhythms—approximately geometric spacing with constant fractional bandwidth—and with the observation that both raw oscillations and their envelopes can carry behaviorally relevant information \cite{penttonenNaturalLogarithmicRelationship2003,buzsaki_brain_2023,donoghueParameterizingNeuralPower2020}. A practical benefit is that decoding can proceed by iterated envelope extraction, which is naturally implemented with constant-\(Q\) filters or analytic-signal envelopes \cite{brownCalculationConstantSpectral1991,boashashEstimatingInterpretingInstantaneous1992}.

% \subsection*{Relation to laminar physiology and predictive signaling}
% The separation of “content” and “gain” implied by HAM dovetails with laminar dissociations in cortex: superficial gamma supports feedforward content, whereas deeper alpha/beta conveys feedback predictions and modulatory signals \cite{kerkoerleAlphaGammaOscillations2014,bastosDCMStudySpectral2015,michalareasCorticalHierarchyFrequencyBased2016}. In LaNMM, PING-like generators act as carriers and JR-like generators provide slow envelope dynamics, furnishing a concrete mechanism for writing and reading nested envelopes across layers in a way that aligns with predictive coding. In this view, predictions arrive as slow envelopes that gate fast error-like content; staged demodulation recovers the message stream at successive time scales.

% \subsection*{Why logarithmic spacing follows from non-overlap}
% Intermodulation produces sideband clusters around each center frequency; to avoid collisions across levels, the span of slower combinations must fit within the guard band of the next-higher cluster. A sufficient and transparent statement of this constraint is the super-increasing inequality \(f_k>\sum_{j>k}f_j\). Geometric (log) spacing with ratio \(r>2\) satisfies the constraint for deep cascades and yields constant \(Q\), making demodulation scale-invariant and reusable across levels \cite{brownCalculationConstantSpectral1991}. Empirically, near-logarithmic spacing of canonical bands is well documented \cite{penttonenNaturalLogarithmicRelationship2003,buzsaki_brain_2023}; HAM supplies an engineering rationale for why such spacing is computationally convenient. Moreover, even the weaker non-overlap condition biases the ensemble toward a log-uniform density of centers, so the main phenomenology does not rely on exact geometric ratios.

% \subsection*{Aperiodic structure from cascades and mixtures}
% Two complementary mechanisms account for aperiodic structure. A single multiplicative cascade with log spacing and small, roughly uniform modulation depth produces a power law because the amplitude of a component involving \(k\) modulators scales as \((m/2)^k\) while \(k\) grows like \(\log_r(1/f)\), giving \(P(f)\propto f^{-\alpha}\) with \(\alpha=2\ln(2/m)/\ln r\). This ties the slope to circuit variables and predicts how changes in modulation depth or spacing should shift \(\alpha\). A second, population-level mechanism arises when oscillator centers are distributed approximately log-uniformly and each contributes narrowband power with constant-\(Q\) width; summing across the bank yields a \(1/f\) backbone in expectation (equal energy per octave) \cite{heScalefreeBrainActivity2014,donoghueParameterizingNeuralPower2020}. Random pairwise modulation on top of a log-uniform pool preserves the \(1/f\) backbone in the interior while introducing edge corrections: a finite low-frequency pedestal from small differences and a high-frequency roll-off from finite support. Which deviation dominates—and thus whether fitted \(\hat\alpha\) flattens or steepens over a finite window—depends on analysis choices such as band edges and constant-Hz versus constant-\(Q\) smoothing. These considerations reconcile the intuition that lower sidebands enrich low frequencies with the analytic expectation that, away from edges, lower and upper sidebands contribute \(\sim 1/f\) per absolute Hz.

% \subsection*{Directed-graph semantics and spectral arithmetic}
% Physiologically, converging inputs sum before they modulate a faster carrier. The directed-graph formulation makes this explicit: the presynaptic drive at a node is a weighted sum of inputs that passes through an envelope nonlinearity and then multiplies the carrier. In the small-signal regime, convergent modulators combine linearly at first order, producing standard AM sidebands at \(f_u\pm f_v\) for each parent \(v\); higher orders generate intermodulation among parents. Recursively expanding along a DAG yields spectra supported on signed linear combinations of ancestor frequencies, which is precisely the arithmetic revealed by frequency tagging \cite{norciaSteadystateVisualEvoked2015,gordonIntermodulationComponentsVisual2019,chenIntermodulationFrequencyComponents2024}. The same formalism clarifies why geometric spacing minimizes incidental collisions among such combinations.

% \subsection*{Methodological considerations and scope}
% Separating periodic peaks from aperiodic background is essential; otherwise, peak leakage biases slope estimates and obscures the dependence of \(\alpha\) on spacing and depth \cite{donoghueParameterizingNeuralPower2020}. Cross-frequency coupling metrics are sensitive to bandwidth choices, surrogates, and waveform shape; constant-\(Q\) analysis, explicit aperiodic–periodic parameterization, and rigorous controls are needed \cite{dvorakProperEstimationPhaseamplitude2014}. Our cascade derivations assume modest modulation depths and narrowband kernels; strong nonlinearities, broad kernels, or spatial mixing will introduce additional harmonics and cross-terms that require model-based correction (e.g., via LaNMM fits). Finally, while HAM provides a constructive route to fractal-like spectra without invoking a phase transition, demonstrating genuine critical dynamics requires perturbational tests and finite-size scaling rather than spectral fits alone \cite{beggsNeuronalAvalanches2003,shewNeuronalAvalanchesPlasticity2013,meshulamCoarseGrainingReveals2020}. 

% Overall, HAM links a simple engineering constraint—guard-banded, constant-\(Q\) spacing—to a demodulable hierarchical code that is compatible with laminar physiology and with the coexistence of discrete oscillatory peaks and a \(1/f^\alpha\) background. This synthesis motivates targeted perturbations and tagging paradigms, combined with constant-\(Q\) analyses and mechanistic LaNMM fitting, to adjudicate between cascade-based and mixture/E–I alternatives in the aperiodic slope.

\section{Conclusion}

HAM proposes a demodulable, guard-banded code in which slower rhythms modulate faster carriers across logarithmically spaced bands, naturally linking spacing and modulation depth to the aperiodic slope while a log-uniform population sets a \(1/f\) baseline. The framework can be mapped onto laminar physiology as an information encoding scheme, yields precise spectral arithmetic in tagging paradigms, and suggests constant-\(Q\) analyses for unbiased inference. As such, it offers a practical route to “spectral engineering” of stimulation and readout that can be tested directly with laminar, ECoG, and MEG recordings.

% \section{Conclusion}

% We introduced \emph{Hierarchical Amplitude Modulation} (HAM) as a compact, demodulable
% encoding scheme in which faster rhythms are multiplicatively gain–modulated by slower
% processes, forming cascades that can be read out as envelopes-of-envelopes. A simple
% spacing requirement for avoiding intermodulation overlap (\(f_k>\sum_{j>k} f_j\)) naturally
% admits geometric (log) spacing with constant fractional bandwidths, enabling a scale-invariant
% chain of bandpass–envelope demodulators. In this setting, two complementary mechanisms
% account for 1/$f^\alpha$ structure: (i) a cascade mechanism that links the aperiodic slope to
% spacing and modulation depth, \(\alpha=2\ln(2/m)/\ln r\), and (ii) a mixture mechanism whereby
% a log-uniform density of narrowband oscillators yields a 1/$f$ backbone in expectation. The
% two mechanisms can coexist and help explain how \(\alpha\) depends both on circuit variables
% (e.g., modulation depth, band spacing) and on analysis choices (e.g., constant-Q vs. constant-Hz
% smoothing and fit windows). At a theoretical level, the coding‑theoretic view (Kraft‑like spacing on a log axis) offers a bridge to algorithmic perspectives on hierarchical representation.

% We instantiated the HAM building blocks—modulation and demodulation—in a laminar
% neural mass model (LaNMM) in which PING-like fast generators act as carriers and JR-like
% slow generators implement envelope extraction. This mapping dovetails with laminar
% physiology and the known frequency segregation of feedforward and feedback channels,
% suggesting how predictive circuits could write and read content in nested envelopes.

% The framework yields concrete, falsifiable predictions: (i) frequency-tagged paradigms should
% exhibit intermodulation arithmetic with amplitudes scaling as products of modulation depths;
% (ii) guard-band (approximately geometric) spacing should minimize cluster collisions and
% facilitate staged demodulation; (iii) laminar readouts should dissociate carriers (superficial,
% gamma) from envelopes (deep, alpha/beta); (iv) within a cascade, manipulations that increase
% modulation depth or decrease spacing should flatten the fitted aperiodic slope in the predicted
% direction; and (v) iterated envelope extraction should reveal the predicted log-spaced content
% with decreasing Q.

% Methodologically, constant-Q preprocessing and explicit separation of aperiodic and periodic
% structure are essential to avoid slope biases and spurious cross-frequency coupling. Conceptually,
% HAM connects engineering constraints (guard-bands, constant-Q filterbanks) with hierarchical
% neural computation and offers a practical route to \emph{spectral engineering} of stimulation and
% analysis protocols. Future work should combine preregistered laminar/ECoG/MEG tests of the
% predictions above with LaNMM-based parameter inference that links changes in the aperiodic
% slope to circuit manipulations, and with controls to dissociate cascade-based accounts from
% E/I or time-scale mixture alternatives.



\section*{Funding}
  Giulio Ruffini, Edmundo Lopez-Sola, and Francesca Castaldo are funded by the European Commission under European Union’s Horizon 2020 research and innovation programme Grant Number 101017716 (Neurotwin) and European Research Council (ERC Synergy Galvani) under the European Union’s Horizon 2020 research and innovation program Grant Number~855109. 
